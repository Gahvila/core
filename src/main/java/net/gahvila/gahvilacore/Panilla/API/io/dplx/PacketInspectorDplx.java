...
import net.gahvila.gahvilacore.Panilla.BukkitPanillaLogger;
...
public class PacketInspectorDplx extends ChannelDuplexHandler {
    ...
    public PacketInspectorDplx(PanillaPlugin panilla, BukkitPanillaPlayer player) {
    ...
    private boolean handlePacketException(BukkitPanillaPlayer player, PacketException e) {
        if (!player.canBypassChecks(panilla, e)) {
            panilla.getInventoryCleaner().clean(player);

            BukkitPanillaLogger panillaLogger = panilla.getPanillaLogger();
